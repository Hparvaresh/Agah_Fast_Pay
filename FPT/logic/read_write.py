...
def make_table_path(table_name, suffix_path: str = None):
    data_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), RuntimeConfig.LOCAL_DATA_PATH
    )
    if not os.path.isdir(data_path):
        os.makedirs(data_path)
    if suffix_path:
        return os.path.join(
            data_path, table_name + suffix_path + PDMappingVO.SUFFIX_CSV
        )
    return os.path.join(data_path, table_name + PDMappingVO.SUFFIX_CSV)


def make_query_select_column_and_sum_groupby(
    table_name: str, date_column: str, sum_column: str, grpup_column: str
) -> str:
...
def read_table_custom1(
    ...
    query = make_query_select_column_and_sum_groupby(
        table_name, date_column, sum_column, grpup_column
    )
...
def read_table_custom2(
    ...
    if df.empty:
        query = make_query_select_from_list(
            table_name, [amount_column, date_column, title_column]
        )

        df = read_table_from_db(query)
        df.to_csv(table_path)

    df_payment_harmony, df_payment_check, df_payment_fastpay = manage_money_title(
        df,
        date_column,
        amount_column,
        title_column,
        sum_output_harmoney_column,
        sum_output_check_column,
        sum_output_fast_pey_column,
    )
...
def read_dollar():
    ...
    dollar_p = list(
        (
            np.array(df_dollar[PDMappingVO.DOLLAR_OPEN])
            + np.array(df_dollar[PDMappingVO.DOLLAR_HIGH])
            + np.array(df_dollar[PDMappingVO.DOLLAR_LOW])
        )
        / 3
    )
    dollar_date = df_dollar[PDMappingVO.DOLLAR_PER].apply(
...
def read_index():
    ...
    df_index[PDMappingVO.DATE_COLUMN] = df_index[PDMappingVO.INDEX_SINCE].apply(
        lambda x: str(x)[:10]
    )
    df_index = df_index.drop(columns=[PDMappingVO.INDEX_SINCE])
    df_index = (
        df_index.groupby(PDMappingVO.DATE_COLUMN)[PDMappingVO.INDEX_VALUE]
        .mean()
        .reset_index()
    )
    df_index = df_index.rename(
        columns={PDMappingVO.INDEX_VALUE: PDMappingVO.INDEX_INDEX_VALUE}
    )
    return df_index
...
def read_IPO():
    ...
    df_IPO[PDMappingVO.DATE_COLUMN] = df_IPO[PDMappingVO.INDEX_SINCE].apply(
        lambda x: str(x)[:10]
    )
